export default function Logo({ variant = 'full', size = 'default', className = '' }) {
  ...
  const IconSVG = () => (
    <div className={`relative shrink-0 ${iconSizes[size] || iconSizes.default}`}>
      ...
      >
        {/* Rounded Burgundy Squircle Container */}
        <rect width="40" height="40" rx="11" fill="#581c2e" />
        
        {/* Upper Exchange Arc (Warm Cream / Ivory Glow) */}
        <path
        ...
        {/* Lower Return Arc (Warm Collegiate Gold) */}
        <path
        ...
        <circle cx="20" cy="20" r="2.5" fill="#fdfbf7" />
      </svg>
    </div>
  );
  ...
  return (
    <div className={`inline-flex items-center gap-2.5 ${className}`}>
      <IconSVG />
      <span className={`font-black tracking-tight font-display text-stone-900 ${textSizes[size] || textSizes.default}`}>
        Campus<span className="text-[#881337]">Swap</span>
      </span>
    </div>
  );
}
